...
export default function PainelSalas({ salas, podeConfigurarLaboratorio, laboratorioProps, minhaEquipe, souAdmin, onAlterarBaiaSala, onAlterarEquipesSala, onCriarSala, onEditarSala, onExcluirSala, onEnviarImagemSala, onRemoverImagemSala, onClose }) {
  const [abrindo, setAbrindo] = useState(null)
  const [editandoSalaDe, setEditandoSalaDe] = useState(null)
          ...
          <div className="config-baias-lista">
            ...
            {salasVisiveis.map(sala => (
              <div key={sala.id} className="painel-salas-linha">
                ...
                {(souAdmin || sala.equipes.includes(minhaEquipe)) && (
                  <button
                    type="button"
...
function CriarSala({ souAdmin, onCriar, onClose }) {
  ...
  const [qtdBaias, setQtdBaias] = useState(9)
  const [equipes, setEquipes] = useState([])
  const [salvando, setSalvando] = useState(false)

  const alternarEquipe = (slug) => {
...
function EditarSala({ sala, onSalvarDetalhes, onSalvarEquipes, onExcluir, onEnviarImagem, onRemoverImagem, onClose }) {
  ...
  const [qtdBaias, setQtdBaias] = useState(sala.qtdBaias)
  const [equipes, setEquipes] = useState(sala.equipes)
  const [salvando, setSalvando] = useState(false)
