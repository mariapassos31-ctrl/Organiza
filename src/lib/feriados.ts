...
function formatarData(ano: number, mes: number, dia: number) {
  return `${ano}-${String(mes).padStart(2, '0')}-${String(dia).padStart(2, '0')}`
}

// Soma/subtrai dias a partir de uma data ano/mes/dia, cruzando mês/ano se preciso.
function deslocarData(ano: number, mes: number, dia: number, delta: number) {
...
const FERIADOS_FIXOS: Array<[number, number, string]> = [
...
const FERIADOS_MOVEIS: Array<[number, string]> = [
  [-47, 'Carnaval'],
  [-2, 'Sexta-feira Santa'],
  [60, 'Corpus Christi'],
]

const cachePorAno = new Map<number, Map<string, string>>()
...
// dataStr: 'YYYY-MM-DD'
export function ehFeriado(dataStr: string): boolean {
...
export function nomeFeriado(dataStr: string): string | null {
  const ano = Number(dataStr.slice(0, 4))
