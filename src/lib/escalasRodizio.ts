...
import type { Participante, BlocoEscala, AvisoEscala } from './tipos'
...
export function getSabados(dataInicio: string, dataFim: string): string[] {
...
// do início.
export function indiceContinuacao(participantes: Array<{ cd_usuario: number | string }>, ultimoUid: string | null | undefined): number {
...
export function construirBlocosRodizio({ participantes, tipo, dataInicio, dataFim, bloco, indiceInicial = 0, sabados }: {
  ...
  let atual: { dtInicio: string; dtFim: string; participanteIdx: number } | null = null
...
export function construirBlocosHibrido({ participantes, dataInicio, dataFim, diasTrabalho, percentualHomeOffice }: {
  ...
  dataInicio: string
  dataFim: string
  diasTrabalho: Array<number | string>
  percentualHomeOffice: number
}): BlocoEscala[] {
  ...
  for (let t = 0; t < n; t++) {
    ...
    let atual: { tipo: string; dtInicio: string; dtFim: string } | null = null
    const fechar = () => {
      if (!atual) return
      blocos.push({
