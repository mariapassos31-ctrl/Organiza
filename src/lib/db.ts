...
export async function equipeIdFromSlug(slug: string | null | undefined): Promise<number | null> {
  if (!slug) return null
...
export async function tecnicoIdsFromUids(
  uids: Array<string | number> | null | undefined,
  ...
  const params: unknown[] = [usuarioIds]
