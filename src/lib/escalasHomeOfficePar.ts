...
function nuncaVaiParaHomeOffice(p: Participante): boolean {
...
export function construirBlocosHomeOfficePar({
  ...
  const abertos = new Map<number | string, { tipo: string; dtInicio: string; dtFim: string }>()
  const blocos: BlocoEscala[] = []

  const fechar = (uid: number | string) => {
  ...
  const marcar = (uid: number | string, dia: string, tipo: string) => {
    const atual = abertos.get(uid)
  ...
  for (let i = 0; i < diasUteis.length; i += tamanhoBloco) {
    ...
    let escolhido: { uids: Array<number | string>; aviso?: string }
    if (vagasDoBloco === 0) {
      escolhido = { uids: [] }
