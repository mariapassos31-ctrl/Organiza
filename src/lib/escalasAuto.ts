...
export async function montarPlanoHibrido({ role, userEquipe, body }: ArgsPlano): Promise<ResultadoPlano> {
  ...
  if (usandoQuantidadeFixa) {
    ...
    const resultado = construirBlocosHomeOfficePar({
      ...
      dataFim,
      diasTrabalho,
      quantidadeHomeOffice: quantidade as number,
      duracaoBlocoDias,
      contagensIniciais,
    ...
    blocos = construirBlocosHibrido({ participantes, dataInicio, dataFim, diasTrabalho, percentualHomeOffice: percentual as number })
    avisos = []
  }
...
export async function montarPlano({ role, userEquipe, body }: ArgsPlano): Promise<ResultadoPlano> {
